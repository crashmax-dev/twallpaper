...
import { curve, positions } from './constants.js'
import type {
  PatternOptions,
  Position,
  RgbColor,
...
export class TWallpaper {
  ...
  updatePattern(pattern: PatternOptions): void {
    ...
    this.container.style.setProperty('--tw-size', size)
    this.container.style.setProperty('--tw-opacity', `${opacity}`)
    this.container.style.setProperty('--tw-blur', `${blur}px`)
    this.container.style.setProperty('--tw-background', background)

    if (image) {
      this.container.style.setProperty('--tw-image', `url(${image})`)
    } else {
      this.container.style.removeProperty('--tw-image')
    }
